...
- **`cli.py`** — Click CLI wiring. Commands: `generate`, `autograde`, `feedback`, `moodle` (group), `student`, `formgrader`. The `moodle` group has subcommands: `export` (merge grades into Moodle CSV, takes assignment name, auto-discovers worksheet from `import/<assignment>.csv`), `fetch` (student downloads assignment), `submit` (student uploads submission), `fetch-submissions` (instructor bulk download), `upload-feedback` (instructor bulk grade push), `feedback` (student views submission status/grade/feedback). Smart output directory defaults infer from nbgrader convention. `--source` auto-discovery. Integrity checking integrated into autograde.
