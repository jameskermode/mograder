...
def _(mo):
    ...
    _mograder_checks = [
        ("Q1: Array creation", "WAIT"),
        ("Q1: Array creation", "PASS"),
        ("Q2: Finite differences", "WAIT"),
        ("Q2: Finite differences", "WAIT"),
        ("Q2: Finite differences", "FAIL"),
        ("Q3: Trapezoidal rule", "WAIT"),
        ("Q3: Trapezoidal rule", "PASS"),
    ]
...
def _(mo):
    ...
    # Auto marks: 25/40
    # Set _mark for manual questions (out of 60), then save.
    _mark = 35  # e.g. _mark = 60
    _feedback = "Decent analysis but missed central vs forward diff discussion."  # e.g. _feedback = "Good analysis of the DP approach..."

    # --- display (do not edit below) ---
    if _mark is not None:
        _total = 25 + _mark
        mo.callout(
            mo.md(f"**Mark: {_total}/100** (auto: 25, manual: {_mark})\n\n{_feedback}"),
            kind="success",
        )
    else:
        mo.callout(
            mo.md(
                "**Awaiting GTA feedback** — edit `_mark` (out of 60) and `_feedback` above\n\n"
                f"Auto marks so far: 25/40"
            ),
            kind="warn",
        )
    return
