...
def _(mo):
    ...
    if _mark is not None:
        _total = 40 + _mark
        mo.callout(mo.md(f"**Mark: {_total}/100** (auto: 40, manual: {_mark})\n\n{_feedback}"), kind="success")
    else:
        mo.callout(mo.md("**Awaiting GTA feedback** — edit `_mark` (out of 60) and `_feedback` above\n\n"
            f"Auto marks so far: 40/40"), kind="warn")
    return
