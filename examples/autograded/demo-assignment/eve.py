...
def _(mo):
    ...
    # Auto marks: 10/10
    # Set _mark for manual questions (out of 90), then save.
    _mark = None       # e.g. _mark = 90
    _feedback = ""     # e.g. _feedback = "Good analysis of the DP approach..."

    # --- display (do not edit below) ---
    if _mark is not None:
        _total = 10 + _mark
        mo.callout(mo.md(f"**Mark: {_total}/100** (auto: 10, manual: {_mark})\n\n{_feedback}"), kind="success")
    else:
        mo.callout(mo.md("**Awaiting GTA feedback** — edit `_mark` (out of 90) and `_feedback` above\n\n"
            f"Auto marks so far: 10/10"), kind="warn")
    return
