...
The formgrader reads `mograder.toml` from the course directory for directory names, Moodle settings, and gradebook path (see [Configuration](../configuration.md)). Options: `--port PORT` to set the server port, `--headless` to suppress the browser.
