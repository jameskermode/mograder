...
2. **[Hub](https://mograder-demo.jrkermode.uk)** — Multi-user student notebook server at `/`. Download assignments, edit in marimo, validate, and submit. The **[Grader](https://mograder-demo.jrkermode.uk/grader)** at `/grader` shows the instructor view (assignments, submissions, grading, students tabs) with pre-populated demo data. The same service also handles the assignment API at `/assignments`. No login required (for a real deployment, authentication should be configured).
...
There is also a **[Demo Workshop](https://jameskermode.github.io/mograder/dashboard/notebooks/demo-workshop.html)** which is a WASM notebook demonstrating hints and encrypted solutions for formative workshops (also available as a hub-hosted workshop for notebooks with native dependencies). The **[Instructor Dashboard](https://mograder-demo.jrkermode.uk/workshop/demo-workshop/dashboard.html#token=mograder-demo-secret)** controls which model solutions are visible to students. The workshop key for this demo is `mograder`.
