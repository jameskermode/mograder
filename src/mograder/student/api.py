...
from mograder.core._utils import add_cors_to_response, match_dir_by_key
...
def create_student_api(course_dir: Path, config: MograderConfig) -> Starlette:
    ...
    def _cors(response: Response) -> Response:
        return add_cors_to_response(
            response, methods="GET, OPTIONS", headers="Content-Type"
        )
    ...
    def _match_release_dir(dir_key: str) -> Path | None:
        return match_dir_by_key(release_dir, dir_key)

    def _match_source_dir(dir_key: str) -> Path | None:
        return match_dir_by_key(source_dir, dir_key)
