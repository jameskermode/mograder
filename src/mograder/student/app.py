...
def _():
    ...
    from mograder.transport.transport import build_transport
...
def _(
    ...
    if not IS_HTTPS and moodle_url and CONFIG.moodle_course_id and get_token():
        try:
            from mograder.transport.moodle_api import (
                sync_assignments as _sync_assignments,
            )
            ...
            if _synced:
                ...
                from mograder.core.config import write_toml as _write_toml
    ...
    if IS_HTTPS:
        if not _https_needs_login:
            ...
            def handle_https_register(_):
                ...
                if not user or not code:
                    ...
                    return
                try:
                    from mograder.transport.https_transport import register
        ...
        _moodle_info = mo.Html(
            f'<div style="font-size:0.85em"><b>Local:</b> <code>{COURSE_DIR}</code>'
            + (f"<br><b>Remote:</b> <code>{moodle_url}</code>" if moodle_url else "")
            + "</div>"
        )
        mo.output.replace(
            mo.hstack(
...
def _(
    ...
    if pending is not None:
        ...
        if _act == "download" and (_client or _transport):
            ...
            if _is_remote:
                from mograder.core.edit_sessions import (
                    spawn_headless_edit,
